...
cos:
    ...
    cos_loop:
            ...
            fmv.w.x fs2, s1
            flt.s s2, fa1, fs2 # Compara sub_pc con 0.001
            bnez s2, end_s2  # Si sub_pc >= 0.001, continúa el bucle

            # current = fa1, expo(x, 2*n + 1) = ft3, expo(-1, n) = ft4, factorial(2*n + 1) = a0

            addi s0, s0, 1 #n++
            # expo(x: base = fa0, 2*n: ex, a3)  = fs3
            li s3, 2
            mul a1, s0, s3 # 2*n
            fmv.s fa0, fs1 # Current
            jal ra expo
            fmv.s fs3, fa0
            ...
            # factorial (2*n) = a0
            li t1, 0x40000000
            fmv.w.x ft1, t1 # 2 in floating point
            fcvt.s.w ft2, s0
            fmul.s fa0, ft1, ft2 # 2*n
            jal ra factorial
            fmv.s fs5, fa0 # f for factorial

            fmul.s ft1, fs3, fs4 # expo1 * expo2
            fdiv.s ft1, ft1, fs5 # expo1 * expo2 / factorial
            fmv.s fa3, fs0 # prev = current
            fadd.s fs0, ft1, fs0 # current = current + operation
            fsub.s fa1, fa3, fs0 # sub_pc = prev - current
            fabs.s fa1, fa1 # |sub_pc|
...
E: 
    li s1, 0x3F800000  # charge 1 in floating point 
